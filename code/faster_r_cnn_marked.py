...
def resize_short_side(
    image: Image.Image, boxes: torch.Tensor, target_short: int
...
class YoloDetectionDataset(Dataset):
    ...
    def _read_labels(
        self, lbl_path: Path, W: int, H: int
        ...
        for ln in lines:
            ...
            xyxy = yolo_to_xyxy(
                np.array([xc, yc, bw, bh], dtype=np.float32), W, H
            )
            x1, y1, x2, y2 = xyxy
...
def train_one_epoch(
    model, optimizer, dataloader, device, epoch, print_freq=10
):
    model.train()
    running = 0.0
    for i, (images, targets) in enumerate(dataloader):
        ...
        if (i + 1) % print_freq == 0:
            ...
            print(
                f"Epoch {epoch} | Iter {i+1}/{len(dataloader)} | loss {avg:.4f} | components: "
                + ", ".join(
...
def evaluate_prf1(
        ...
        for images, targets in dataloader:
            ...
            for out, tgt in zip(outputs, targets):
                ...
                for c in range(1, num_classes + 1):
                    p_idx = (plabels == c).nonzero(
                        as_tuple=False
                    ).flatten()
                    g_idx = (glabels == c).nonzero(
                        as_tuple=False
                    ).flatten()
                    pb = pboxes[p_idx]
                    gb = gboxes[g_idx]
                    matched_pred, fp_set, fn_set = _greedy_match_iou(
                        pb, None, gb, iou_thresh=iou_thresh
                    )
    ...
    micro_f1 = (
        2 * micro_p * micro_r / max(1e-8, (micro_p + micro_r))
...
def evaluate_pr_auc(
    ...
    for thr in thrs:
        ...
        for pred, gt in zip(all_preds, all_gts):
            ...
            for c in range(1, num_classes + 1):
                ...
                mp, fp, fn = _greedy_match_iou(
                    pb[p_idx], None, gb[g_idx], iou_thresh=iou_thresh
                )
                TP += len(mp)
                FP += len(fp)
                FN += len(fn)
    ...
    return {
        ...
        "pr_curve": {
            "recall": recs.tolist(),
            "precision": precs.tolist(),
        },
    }


def save_pred_images(
    model,
    dataloader,
    device,
    class_names,
    ...
    print(f"[VIS] Generating visualization images into: {out_dir}")
    ...
    with torch.no_grad():
        for images, targets in dataloader:
            ...
            for img, tgt, out in zip(images, targets, outputs):
                if idx >= max_images:
                    print(f"[VIS] Reached max_images={max_images}, stop.")
                    return

                img_u8 = (img * 255).clamp(0, 255).byte().cpu()
                ...
                if boxes.numel() == 0:
                    pil_img = to_pil_image(img_u8)
                    save_path = os.path.join(
                        out_dir, f"pred_{idx:04d}_nobox.jpg"
                    )
                    pil_img.save(save_path)
                    print(f"[VIS] Saved (no boxes): {save_path}")
                    idx += 1
                    continue

                label_strs = [
                    f"{class_names[int(c) - 1]}:{float(s):.2f}"
                ...
                pil_img = to_pil_image(vis)
                save_path = os.path.join(out_dir, f"pred_{idx:04d}.jpg")
                pil_img.save(save_path)
    ...
    print(f"[VIS] Done. Total saved images: {idx}")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--data_root", type=str, default="./dataset", help="dataset root"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--imgsz", type=int, default=1024, help="short side image size"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="validation ratio (unused here)",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
    ...
    parser.add_argument(
        "--eval_map",
        action="store_true",
        help="compute mAP/mAR after each epoch",
    )
    parser.add_argument(
        "--eval_prf1",
        action="store_true",
        help="compute P/R/F1 after each epoch",
    )
    parser.add_argument(
        "--eval_prauc",
        action="store_true",
        help="compute PR-AUC after training",
    )
    parser.add_argument(
        "--iou_eval",
        type=float,
        default=0.5,
        help="IoU threshold for PR/F1 and PR-AUC",
    )
    parser.add_argument(
        "--conf_thr",
        type=float,
        default=0.05,
        help="score threshold for PR/F1 and visualization",
    )
    ...
    train_ds = YoloDetectionDataset(
        data_root, "train", class_names, imgsz=args.imgsz
    )
    val_ds = YoloDetectionDataset(
        data_root, "valid", class_names, imgsz=args.imgsz, hflip_p=0.0
    )
    ...
    for epoch in tqdm(range(1, args.epochs + 1), desc="Epochs"):
        ...
        if args.eval_prf1:
            prf1 = evaluate_prf1(
                ...
                val_loader,
                device,
                iou_thresh=args.iou_eval,
                score_thr=args.conf_thr,
                num_classes=num_classes,
            )
            print(
                "[P/R/F1 micro]",
                {k: round(v, 4) for k, v in prf1["micro"].items()},
            )

        ckpt_path = (
            Path(args.save_dir) / f"epoch{epoch:03d}_val{val_loss:.4f}.pth"
        )
        torch.save(
            {
        ...
        vis_dir_epoch = os.path.join(
            args.save_dir, f"vis_epoch{epoch:03d}"
        )
        save_pred_images(
            model=model,
